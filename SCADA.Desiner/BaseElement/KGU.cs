...
{
    /// <summary>
    /// класс контроля габарита устройства
    /// </summary>
    public class KGU : Shape, IGraficObejct, IScrollElement, IFreeAngle
    {
        ...
        ViewElement viewElement = ViewElement.kgu;
        /// <summary>
        /// Вид элемента
        /// </summary>
        ...
        public KGU(double kscroll, int stationnumber, double X, double Y)
        {
            StationNumber = stationnumber;
            GeometryFigure(kscroll, X, Y);
        }

        public KGU(int stationnumber, PathGeometry geometry, double strokethickness, string name, double currencyScroll)
        {
            StationNumber = stationnumber;
        ...
        public SizeElement SizeGeometry()
        {
            ...
            return new SizeElement();// { Height = Math.Round(Math.Abs(Figure.Figures[0].StartPoint.Y - P2.Y) / scroll), Widht = Math.Round(Math.Abs(Figure.Figures[0].StartPoint.X - P1.X) / scroll) };
        }
