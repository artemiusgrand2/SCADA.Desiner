...
namespace SCADA.Desiner.BaseElement
{
    ...
    /// </summary>
    class LineHelp : Shape, IGraficObejct, IDeleteElement, IShowSettings, IFreeAngle
    {
        ...
        public Brush ColorStrokeDefult
        {
            get
            {
                return _colordefultstroke;
            }
            set
            {
                _colordefultstroke = value;
            }
        }
        NameColors _namecolor = new NameColors();
        /// <summary>
        /// имя цвет для данного элемента
        /// </summary>
        public NameColors NameViewColor
        {
            get
            {
                return _namecolor;
        ...
        ViewElement viewElement = ViewElement.line;
        /// <summary>
        /// Вид элемента
        /// </summary>
        ...
        public LineHelp(int stationnumber)
        {
            CreateTransform();
            GetNameColorNull();
        ...
        private void SetSettings(int stationnumber, int stationRight, PathGeometry geometry, double strokethickness, string namecolor, byte r, byte g, byte b, bool isFillInside)
        {
            ...
            // 
            GeometryFigureOpen(geometry, strokethickness);
            UpdateRamkaAllocation();
        }
        ...
        private void CreateTransform()
        {
            ...
            RenderTransform = group_transform;
        }

        private void GetNameColorNull()
        {
            _namecolor.NameColor = string.Empty;
            _namecolor.R = ((SolidColorBrush)_colordefultstroke).Color.R;
            _namecolor.G = ((SolidColorBrush)_colordefultstroke).Color.G;
            _namecolor.B = ((SolidColorBrush)_colordefultstroke).Color.B;
        }
            ...
            for (int i = 0; i < _figure.Figures.Count; i++)
            {
                ...
                for (int j = 0; j < _figure.Figures[i].Segments.Count; j++)
                {
                    ...
                    if ((lin.Point.X == CurrentLine.X2 && lin.Point.Y == CurrentLine.Y2))
                    {
                        ...
                        OperationsGrafic.FramePoint = new Rectangle() { Width = StrokeThickness * _sctollrectangle, Height = StrokeThickness * _sctollrectangle, Stroke = Brushes.Red, StrokeThickness = 2 * OperationsGrafic.SaveScroll };
        ...
        public Index FindIndexLine(Line CurrentLine)
        {
            ...
            for (int i = 0; i < _figure.Figures.Count; i++)
            {
                ...
                {
                    OperationsGrafic.FramePoint = new Rectangle() { Width = StrokeThickness * _sctollrectangle, Height = StrokeThickness * _sctollrectangle, Stroke = Brushes.Green, StrokeThickness = 2 * OperationsGrafic.SaveScroll };
        ...
        private void GeometryFigure()
        {
            ...
            _strokethickness *= OperationsGrafic.SaveScroll;
            StrokeThickness = _strokethickness;
        }

        private void GeometryFigureCopy(PathGeometry geometry, double strokethickness/*, double scroll*/)
        {
            ...
            Stroke = _colordefultstroke;
            StrokeThickness = strokethickness;//* scroll;
            _strokethickness = strokethickness;
        }

        private void GeometryFigureOpen(PathGeometry geometry, double strokethickness)
        {
            foreach (PathFigure geo in geometry.Figures)
            {
                PathFigure newfigure = new PathFigure() {IsClosed = false };
            ...
            Stroke = _colordefultstroke;
            _strokethickness = strokethickness * OperationsGrafic.SaveScroll; 
            StrokeThickness = _strokethickness;
        ...
        public void SetStroke(double Weight/*, double scroll*/)
        {
            _strokethickness = Weight;
            StrokeThickness = _strokethickness;
            UpdateRamkaAllocation();
            //изменяем размер области захвата
            try
            {
                if (OperationsGrafic.FramePoint != null)
                {
                    Point zenterpoint = new Point(OperationsGrafic.FramePoint.Margin.Left + OperationsGrafic.FramePoint.Width / 2, OperationsGrafic.FramePoint.Margin.Top + OperationsGrafic.FramePoint.Height / 2);
                    OperationsGrafic.FramePoint.Width = _sctollrectangle * StrokeThickness;
                    OperationsGrafic.FramePoint.Height = _sctollrectangle * StrokeThickness;
                    OperationsGrafic.FramePoint.Margin = new Thickness(zenterpoint.X - OperationsGrafic.FramePoint.Width / 2, zenterpoint.Y - OperationsGrafic.FramePoint.Height / 2, 0, 0);
                }
            }
            catch { }
        }
